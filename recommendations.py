...
def test_sim_distance():
    for name1 in critics.keys():
        for name2 in critics.keys():
            if name1 != name2:
                print(sim_distance(critics,name1,name2))
...
def test_sim_pearson():
    for name1 in critics.keys():
        for name2 in critics.keys():
            if name1 != name2:
                print(sim_pearson(critics,name1,name2))
...
def test_transformPrefs():
    movies = transformPrefs(critics)
    print(topMatches(movies,'Superman Returns'))


def test_transformPrefs_getRecommendations():
    movies = transformPrefs(critics)
...
def test_GetRecommendedItems():
    itemsim = calculateSimilarItems(critics)
    print(getRecommendedItems(critics,itemsim,'Toby'))
    
    
def test_loadMoviesLens():
    prefs = loadMovieLens()
    print(calculateSimilarItems(prefs,n=50))
